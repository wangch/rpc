...
func suitableEvent(typ reflect.Type, reportErr bool) map[string]*methodType {
	...
	for m := 0; m < typ.NumMethod(); m++ {
		...
		// first is *Event
		if returnType := mtype.Out(0); returnType != typeOfEvent {
			if reportErr {
				log.Println("method", mname, "returns", returnType.String(), "not *Event")
			}
			continue
		}
		// second is error
...
func (s *service) sub(server *Server, sending *sync.Mutex, mtype *methodType, req *Request, argv reflect.Value, codec ServerCodec) {
	mtype.Lock()
	mtype.numCalls++
	mtype.Unlock()
